...
const MessagesPage = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (!token) return;
        const res = await apiClient.get('/api/users/me', { headers: { Authorization: `Bearer ${token}` } });
        const me = res.data;
        const list = (me?.friends || []).map((f: any) => ({ id: f._id || f.id || f, name: f.fullName || '', nickname: f.nickname || '' }));
        setFriends(list);
      } catch {}
    })();
  }, [token]);
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return friends;
    return friends.filter((f) => (f.nickname || '').toLowerCase().includes(q) || (f.name || '').toLowerCase().includes(q));
  }, [friends, search]);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}> 
        ...
        <Text style={styles.subtitle}>Chat via streaks and compare results</Text>
        <TextInput value={search} onChangeText={setSearch} placeholder="Search friends" placeholderTextColor="#9CA3AF" style={styles.search} />
      </View>
