...
const HubPage = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (!token) return;
        const res = await apiClient.get("/api/users/students", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setStudents(res.data || []);
        const fr = await apiClient.get("/api/friendships", {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.rating}>
              {item.rating}% • {item.attempts} tests
            </Text>
          </View>
        </View>
        {isFriend ? (
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              try {
                ...
                setFriendIds(
                  new Set([...Array.from(friendIds), String(item.id)])
                );
              } catch {}
            }}
          >
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          Find classmates, compare progress, and connect
        </Text>
        <TextInput
          value={search}
